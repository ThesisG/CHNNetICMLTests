...
for arch in range(init, init + archs):
    ...
    # store results
    with open(f"BNG PWLinear/BNG_pwLinearArch{arch+1}.txt", "w") as metFile:
        # FNN results
    ...
    for seed in range(num_seeds):
        ...
        plt.title(f"BNG PWLinear: Architecture {arch+1}")
        plt.xlabel("Epoch")
        plt.ylabel("Loss")
        plt.legend(["FNN"] + ["CHN"])
        plt.savefig(f"BNG PWLinear/bng_pwLinearArch{arch+1}Seed{seed+1}.pdf")
        plt.clf()
