...
# fetch dataset
X, y = fetch_data('1191_BNG_pbc', return_X_y=True, local_cache_dir='./Datasets')
...
for arch in range(init, init + archs):
    ...
    # store results
    with open(f"1191_BNG_pbc/1191_BNG_pbcArch{arch}.txt", "w") as metFile:
        # FNN results
    ...
    for seed in range(num_seeds):
        ...
        plt.title(f"1191_BNG_pbc: Architecture {arch}")
        plt.xlabel("Epoch")
        plt.ylabel("Loss")
        plt.legend(["FNN"] + ["CHN"])
        plt.savefig(f"1191_BNG_pbc/1191_BNG_pbcArch{arch}Seed{seed}.pdf")
        plt.clf()
