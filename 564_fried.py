...
# fetch dataset
X, y = fetch_data('564_fried', return_X_y=True, local_cache_dir='./Datasets')
...
for arch in range(init, init + archs):
    ...
    # store results
    with open(f"564_fried/564_friedArch{arch}.txt", "w") as metFile:
        # FNN results
    ...
    for seed in range(num_seeds):
        ...
        plt.title(f"564_fried: Architecture {arch}")
        plt.xlabel("Epoch")
        plt.ylabel("Loss")
        plt.legend(["FNN"] + ["CHN"])
        plt.savefig(f"564_fried/564_friedArch{arch}Seed{seed}.pdf")
        plt.clf()
