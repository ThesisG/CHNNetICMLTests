...
# fetch dataset
X, y = fetch_data('1201_BNG_breastTumor', return_X_y=True, local_cache_dir='./Datasets')
...
for arch in range(init, init + archs):
    ...
    for seed in range(num_seeds):
        ...
        CHN_model.add(Dense(1, activation="linear"))
    ...
    # store results
    with open(f"1201_BNG_breastTumor/1201_BNG_breastTumorArch{arch}.txt", "w") as metFile:
        # FNN results
    ...
    for seed in range(num_seeds):
        ...
        plt.title(f"1201_BNG_breastTumor: Architecture {arch}")
        plt.xlabel("Epoch")
        plt.ylabel("Loss")
        plt.legend(["FNN"] + ["CHN"])
        plt.savefig(f"1201_BNG_breastTumor/1201_BNG_breastTumorArch{arch}Seed{seed}.pdf")
        plt.clf()
