...
# fetch dataset
X, y = fetch_data('connect_4', return_X_y=True, local_cache_dir='./Datasets')
...
for arch in range(init, init + archs):
    ...
    for seed in range(num_seeds):
        ...
        CHN_model.add(Dense(3, activation="softmax"))
    ...
    # store results
    with open(f"Connect/connectArch{arch+1}.txt", "w") as metFile:
        # FNN results
    ...
    for seed in range(num_seeds):
        ...
        plt.title(f"Connect: Architecture {arch+1}")
        plt.xlabel("Epoch")
        plt.ylabel("Loss")
        plt.legend(["FNN"] + ["CHN"])
        plt.savefig(f"Connect/connectArch{arch+1}Seed{seed+1}.pdf")
        plt.clf()
