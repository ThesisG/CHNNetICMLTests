...
# fetch dataset
X, y = fetch_data('1203_BNG_pwLinear', return_X_y=True, local_cache_dir='./Datasets')
...
for arch in range(archs):
    ...
    # store results
    with open(f"1203_BNG_pwLinear/1203_BNG_pwLinearArch{arch}.txt", "w") as metFile:
        # FNN results
    ...
    for seed in range(num_seeds):
        ...
        plt.title(f"1203_BNG_pwLinear: Architecture {arch}")
        plt.xlabel("Epoch")
        plt.ylabel("Loss")
        plt.legend(["FNN"] + ["CHN"])
        plt.savefig(f"1203_BNG_pwLinear/1203_BNG_pwLinearArch{arch}Seed{seed}.pdf")
        plt.clf()
