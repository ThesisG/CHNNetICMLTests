...
# fetch dataset
X, y = fetch_data('sleep', return_X_y=True, local_cache_dir='./Datasets')
...
for arch in range(init, init + archs):
    ...
    for seed in range(num_seeds):
        ...
        CHN_model.add(Dense(5, activation="softmax"))
    ...
    # store results
    with open(f"Sleep/sleepArch{arch+1}.txt", "w") as metFile:
        # FNN results
    ...
    for seed in range(num_seeds):
        ...
        plt.title(f"Sleep: Architecture {arch+1}")
        plt.xlabel("Epoch")
        plt.ylabel("Loss")
        plt.legend(["FNN"] + ["CHN"])
        plt.savefig(f"Sleep/sleepArch{arch+1}Seed{seed+1}.pdf")
        plt.clf()
